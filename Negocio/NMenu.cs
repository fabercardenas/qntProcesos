...
namespace Negocio
{
    public class NMenu : Datos.DSQL 
    {
        ...
        public DataTable consultaSubMenu(Int32  idPadre ,string   idPerfil )
        {
            if (idPerfil == "0")
            {
                return queryTable("SELECT * FROM app_MenuGrupo " + 
                           "WHERE (app_MenuGrupo.Id_PadreItem = '" + idPadre + "') AND app_MenuGrupo.Estado=1 AND app_MenuGrupo.Tipo='interno6' " +
        ...
        public DataTable  consultaPadresImagen(string  idPerfil, string Tipo ) 
        {
            if (idPerfil == "0")
                return queryTable("SELECT CONVERT(varchar(50), NEWID()) temporal, * FROM app_MenuGrupo WHERE app_MenuGrupo.Id_PadreItem IS NULL AND app_MenuGrupo.Tipo IN('" + Tipo + "') AND app_MenuGrupo.Estado=1 AND app_MenuGrupo.Imagen IS NOT NULL ORDER BY app_MenuGrupo.orden ASC");
            else
                return queryTable("SELECT CONVERT(varchar(50), NEWID()) temporal, * FROM app_MenuGrupo INNER JOIN app_MenuPerfil ON app_MenuGrupo.Id_MenuItem = app_MenuPerfil.Id_ItemGrupo WHERE (app_MenuGrupo.Id_PadreItem IS NULL) AND (app_MenuGrupo.Estado = 1)  AND (app_MenuGrupo.Imagen IS NOT NULL) AND app_MenuGrupo.Tipo IN('" + Tipo + "') AND (app_MenuPerfil.Id_Perfil = '" + idPerfil + "') ORDER BY app_MenuGrupo.orden ASC");
        }
